...
const zhCN: Record<string, string> = {
    ...
    'When using "Convert all virtual links to real links (preview)", virtual links that point to more than one note are skipped so you can convert them one by one manually. When off, they are included and checked by default, converting only the first target.': '使用「将笔记中所有虚拟链接转换为真实链接（预览）」时，指向多个笔记的虚拟链接会被跳过，方便你手动逐一转换。关闭后它们会包含在列表中并默认勾选，转换时仅取第一个目标。',
