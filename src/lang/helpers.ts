...
const zhCN: Record<string, string> = {
    ...
    'When enabled, excluded keywords only apply to notes with the frontmatter property set to true. When disabled, excluded keywords apply to all notes. Usage: add "fakelink-exclude: true" to a note\'s frontmatter to enable exclusion for that note.': '启用后，排除关键词仅对 frontmatter 中设置了该属性的笔记生效。关闭后全局生效。用法：在笔记 frontmatter 中添加 "fakelink-exclude: true" 即可对该笔记启用排除。',
    'Frontmatter exclusion property': 'Frontmatter 排除属性名',
    'The frontmatter property name to check. Only notes with this property set to true will have excluded keywords applied.': '要检查的 frontmatter 属性名。仅当笔记的此属性设为 true 时，排除关键词才会对该笔记生效。',
    'Enable frontmatter exclude list': '启用 Frontmatter 排除列表',
    'When enabled, each note can define additional excluded keywords in its frontmatter as a list. These add on top of the global excluded keywords. Usage: add "fakelink-exclude: [keyword1, keyword2]" to a note\'s frontmatter.': '启用后，每篇笔记可在 frontmatter 中额外定义排除关键词列表，与全局排除关键词叠加生效。用法：在笔记 frontmatter 中添加 "fakelink-exclude: [关键词1, 关键词2]"。',
