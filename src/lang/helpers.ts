...
const zhCN: Record<string, string> = {
    ...
    'When enabled, FakeLink intercepts clicks on obsidian://adv-uri links that are rendered as regular clickable links (<a>) and jumps to the target line itself, so line links work even without the Advanced URI plugin. In edit mode Obsidian renders such links without a clickable element, so there line jumping is handled by the Advanced URI plugin (if installed) instead. FakeLink never registers the protocol handler, so the two plugins coexist without conflict. Generate line links via the right-click menu "Copy line link (adv-uri)".': '启用后，FakeLink 会拦截渲染为普通可点击链接（<a>）的 obsidian://adv-uri 链接并自行跳转到目标行，即使未安装 Advanced URI 插件也能跳行。在编辑模式下，Obsidian 将这类链接渲染为不可点击的元素，因此该模式下的行跳转由 Advanced URI 插件（若已安装）处理。FakeLink 不会注册协议，两个插件可共存不冲突。生成行链接请使用右键菜单「复制行链接（adv-uri）」。',
